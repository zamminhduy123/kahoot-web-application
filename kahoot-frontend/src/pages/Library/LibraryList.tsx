...
const LibraryList = ({}: LibraryListProps) => {
	...
	useEffect(() => {
		getAllGames().then(
			(res : any) => {
				if (res && res.data)
				dispatch(setNewLibrary(res.data.games.map((d : any) => {
					return {
						_id: d._id,
						title: d.title,
						owner: d.onwer,
	...
	React.useEffect(()=>{
		if (searchKeyWord)
			setDisplayGames(games.filter(d => 
	...
	return (
		<Container maxW="container.lg" w='100%' flex={1}>
			<Stack justifyContent="center">
				...
				{displayGames.length ? displayGames.map((data: any, index: number) => {
					return (
						<LibraryItem
							id={index}
							key={`${data._id}-${index}`}
				...
				}): <Center width='100%'><img width={'400px'} src='https://img.freepik.com/free-vector/no-data-concept-illustration_114360-536.jpg?t=st=1658418266~exp=1658418866~hmac=e403902abc854ef3f33d8ca6043dac4fb68a8687335b727a3478c23aab132f49&w=826'/></Center>}
			</Stack>
		</Container>
	)
}
